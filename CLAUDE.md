...
**Not built yet**, roughly in value order:
  ...
  * **Replace-mode scheduled sync.** Add-only sync shipped 2026-08-26; this is the second half.
    `remove_tracks/4` is the piece that was missing and it is now in place and spent twice, on
    corrections and on Replace. Sync is the third caller: a destination track whose source track
    is gone should go too, which is the difference between Soundiiz's Add and Replace modes —
    see `docs/reference/domain.md`. The column it needs is a mode on `syncs`; everything else
    exists.
