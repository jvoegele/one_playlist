...
| `mix bond.audit` | Which public functions carry a Bond contract and which carry none — the one thing `Bond.Coverage` structurally cannot report, since its table only lists assertions that *ran*. `--verbose` names them, `--only` filters by module, `--min N` exits non-zero below N% for a ratchet. **40% here today.** Reporting only, deliberately outside `precommit`: a bare function is a question, not a defect. Shipped in bond 1.19.0 from this project's prototype — see `docs/library-feedback.md`. |
