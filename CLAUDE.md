...
| `mix contracts.audit` | Which public functions carry a Bond contract and which carry none — the one thing `Bond.Coverage` structurally cannot report, since its table only lists assertions that *ran*. `--verbose` names them; `--min N` exits non-zero below N%, for a ratchet. Reporting only, deliberately outside `precommit`. **Superseded once bond ships `mix bond.audit`** — see `docs/library-feedback.md`; the library version reads a compile-time reflection instead of parsing source, and this local task can then be deleted. |
...
| Match quality | Four corpora, all replayable offline. `dev/measure/replay.exs`: **82 certain, 12 duration-corroborated, 5 none, 1 wrong** of 100 random MusicBrainz recordings. `dev/corpus/replay_credit_cases.exs`: **96 correct, 12 equivalent, 7 missed, 0 wrong** of 115 hard credit cases, and **5 of 5** hand-labelled decline cases correctly declined. `dev/corpus/replay_enrichment.exs` (255 cases from the library **and** `dev/playlists/*.csv`): **149 correct, 30 equivalent, 32 unverified, 23 missed, 0 wrong** of 234 labelled — re-harvested 2026-08-28 to carry `release_tags`, and **zero wrong for the first time**. The corpus now has a `tags withheld` control row, which is how that is known to be the tags rather than the capture: identical candidates minus every tag score 147/31/32/23/**1**, so `:live`/`:remix`/`:demo` are worth +2 correct and −1 wrong at **no cost in misses**. The feared over-veto — `conflict?/2` is strict set equality over a union across every release a recording appears on — did not appear. See `docs/reference/domain.md` |
