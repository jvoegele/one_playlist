defmodule OnePlaylist.Library.Recording do
  ...
  `OnePlaylist.Library.find_or_create/1` joins on the ISRC and *only* on the
  ISRC — never on a title, because a wrong join is not undoable by adding — so
  `isrc_is_canonical` below is what stands between that join and a library
  quietly holding two of everything.
  ...
  """
  # Written as `normalize(x) == x` rather than against `track.isrc`: it says
  # "whatever came out is already canonical", which is the property the join
  # needs and is true of `nil` too.
  #
  ...
  `OnePlaylist.Providers.Library.playlist_track_ids/3` reports these ids and
  `Transfers.Runner`'s snapshot-and-diff compares against them, so a track
  carrying anything else is a transfer that cannot tell what the destination
  already holds — every run adding every track again, reported as a success.
  `OnePlaylist.Library.PlaylistItem.to_track/2` carries the same law at the
  other end of the same identity.
  """
  # Proven by mutation: `provider_id: recording.musicbrainz_recording_id` fires
  # it, and is the plausible edit — that field is also an identity, and is the
  # more correct-looking of the two.
