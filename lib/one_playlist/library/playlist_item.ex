defmodule OnePlaylist.Library.PlaylistItem do
  ...
  **Nothing structural is editable, and the postcondition below is what keeps it
  that way.** `OnePlaylist.Library.update_item/4` casts exactly this list, so a
  structural field appearing here becomes a form that can rewrite the row's place
  in the playlist or — worse — its `recording_id`, silently re-linking the item to
  any recording in a shared, ownerless table by posting a field the form never
  rendered. Nothing raises; the item simply becomes a different track.
  """
  # Proven by mutation: adding `:recording_id` to `@owned` fires it.
  ...
  Reaching for the item's own `id` instead — it is right there, and it is the
  obvious thing to reach for — breaks a transfer *quietly*: two items of one
  recording produce two tracks with different ids, `Runner`'s snapshot-and-diff
  sees neither as already present, and the destination gains the track twice
  with the report reading as a success. `identifies_the_recording` below is that
  sentence made checkable.

  `the_items_own_account_wins` is the precedence rule above, restated over the
  finished pair rather than trusted to `with_recording/2` — the law is about what
  a caller gets back, and the merge has two steps.
  """
  # Proven by mutation: `provider_id: item.id` fires the first, and taking the
  # title from `recording` fires the second.
  ...
  Both of those claims are checked below. `fills_only_gaps_*` is the *Throw Your
  Arms Around Me* bug as a law, and it names only ISRC and length because those
  are the only two fields either side may supply — everything else is read from
  the recording unconditionally and is a fact about the recording rather than a
  precedence rule.
  """
  # `idempotent` is last so the cheaper assertions fail first, and it is sound
  # only because Meyer's Assertion Evaluation rule suppresses contracts while an
  # assertion runs — which is what stops it recursing.
  #
  # Proven by mutation: reversing either `||` fires the matching
  # `fills_only_gaps`; appending to the title fires `idempotent`.
