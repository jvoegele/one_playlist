defmodule OnePlaylist.Transfers.Transfer do
  ...
  *transition* rather than a value: it relates the result to the argument, which
  ...
  up — and `OnePlaylistWeb.TransferLive.Show` renders it as "150% of the source"
  rather than as an error. The counters are independent columns rather than
  values derived from one list, so it is reachable from any write that skips
  `record_matched/2`: a migration, a manual fix, a future writer. A re-run
  accumulating onto the previous run's numbers gets there in one step — six
  matched of three total — which is what `reset_counters/1` prevents.

  Raising is the intended behaviour rather than a regrettable side effect. The
  alternative is rendering a number this application knows to be false.
  """
  # `>=` on the lower bound rather than `>`: zero matched of ten is a real and
  # reportable outcome.
