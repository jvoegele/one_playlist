...
at all to "which functions have none?" — which the usage rules themselves single out as the
dangerous case, precisely because nothing shows it:

> The failure mode is one-sided: a codebase with too few contracts looks exactly like one that
> never needed them, so careful screening under-contracts by default and nobody notices.

**Why it mattered.** This project believed itself well contracted. Measured, it was at **40%** —
136 of 336 public non-callback functions, with 200 carrying nothing. Nobody had been able to see
that, because the only instrument reports the opposite half.

**What was built.** `mix contracts.audit` here, as a prototype that parses `lib/**/*.ex`; and then
the real version in the library, on branch `feature/contract-density-audit`
(`../bond`, off 1.18.1).

The library version is better in a way this project could not be, and the difference is the point
of reporting it rather than keeping the local task. Bond's compiler already builds an `annotated`
list of every function it contracted and then **discards it** — `build_boundaries_reflection/1`
keeps only numeric precondition boundaries from that same list. Emitting a
`__bond_contracted__/0` reflection alongside costs one more function and makes the audit exact,
where a source parser cannot see a function generated by a macro, a module that acquired
`use Bond` through some other `__using__`, or a `defdelegate`, and has to guess at
`elixirc_paths`.

Both were run over this application and agree on the headline — 40% — while the reflection-based
one covers more ground: 481 public functions to the parser's 336, and 196 callbacks to 102,
because it reads `@behaviour` attributes rather than `@impl` and so catches `GenServer`,
`Oban.Worker` and `Phoenix.LiveView` callbacks the parser missed.

**Three design points worth keeping if this is upstreamed differently.**

  * **Emit the reflection even for a module that contracted nothing.** `%{}` is how an audit tells
    "uses Bond, contracts nothing" — the most interesting row it can print — from "does not use
    Bond", which is a different question with a different answer.
  * **Separate a function's own `@pre`/`@post` from one covered only by a module `@invariant`.**
    Both are coverage and they are not the same claim: an invariant is a law about the *type*, not
    a promise about any one function, and merging them flatters the number.
  * **Exclude machine-generated exports by convention, not by list** — a name both starting and
    ending in `__`. Counting `__schema__/1` and `__changeset__/0` inflated the denominator by 64
    here; a project with forty Ecto schemas would report a third of the truth.

And one trap that cost a wrong number twice: **decide coverage per function name, not per
`{name, arity}`.** A default argument exports an arity nobody wrote — `def f(a, b \\ x)` exports
`f/1` and `f/2` — Bond contracts one, and the other reads as bare. That was 64 of 395 supposed
gaps on this codebase.
